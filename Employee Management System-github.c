...
if((fp=fopen("employeeInfo.txt","rb+"))==NULL) //Shubham_Sharma
{
...
char username[10],password[10]; //Shubham_Sharma
printHead();
welcome();
printHead();
printf("\n\t\t\t\t  Login Screen");
...
i=0;
	...
	{
		password[i] = getch();
		if(password[i] == 13 )//Shubham_Sharma
		{
			break;
		}
...
if((ft=fopen("temp.txt","wb+"))==NULL)//Shubham_Sharma
{
    printf("\n\n\t\t\t\\t!!! ERROR !!!\n\t\t");
    system("pause");
     return fp;
...
if(flag==1)
    ...
    fflush(stdin);
    fgets(e.name,100,stdin); //fgets takes an extra \n character as input
    e.name[strlen(e.name)-1]='\0';//Shubham_Sharma
    ...
    printf("\n\n\t\tEnter Branch: ");
    fflush(stdin);
    fgets(e.branch,50,stdin);//Shubham_Sharma
    e.branch[strlen(e.branch)-1]='\0';
    ...
    printf("\n\n\t\tEnter Permanant Address: ");
    fflush(stdin);
    fgets(e.prtaddr,200,stdin);//Shubham_Sharma
    e.prtaddr[strlen(e.prtaddr)-1]='\0';
    ...
    fwrite(&e,sizeof(e),1,fp);//Shubham_Sharma
}
...
//================SEARCH EMPLOYEE==
void searchRecord(FILE *fp)//Shubham_Sharma
{printHead();
printf("\n\t\t\t\Search Employee");
...
if(flag==1)
    ...
    printf("\n\t\tNAME : %s",e.name);
        printf("\n\n\t\tID : %d",e.id);
        printf("\n\n\t\tDESIGNATION : %s",e.desgn);
        printf("\n\n\t\tBRANCH : %s",e.branch);//Shubham_Sharma
        printf("\n\n\t\tSALARY: %.2f",e.sal);
        printf("\n\n\t\tPRESENT ADDRESS : %s",e.psaddr);
        printf("\n\n\t\tPERMANANT ADDRESS : %s",e.prtaddr);
        printf("\n\n\t\tPHONE : %s",e.phone);
...
{
    printHead();
     printf("\n\t\t\t\List of Employee From Dhaka");//Shubham_Sharma
    Employee e;
    int i,siz=sizeof(e);
...
void mainbr(FILE * fp)
{
    ...
    while((fread(&e,siz,1,fp))==1)//Shubham_Sharma
    {
        if((strcmp(e.branch,"Main")==0)||(strcmp(e.prtaddr,"main")==0)||(strcmp(e.prtaddr,"MAIN")==0))
        {
        printf("\n\n\t\tID : %d",e.id);
        printf("\n\n\t\tNAME : %s",e.name);
        printf("\n\n\t\tDESIGNATION : %s",e.desgn);//Shubham_Sharma
        printf("\n\n\t\tPHONE : %s",e.phone);
        printf("\n\n\t\tE-MAIL : %s\n\t",e.mail);
        printChar('=',65);
        }
...
{
    printHead();
    printf("\n\t\t\t\List of Employee of Other Branch");//Shubham_Sharma
    Employee e;
    int i,siz=sizeof(e);
